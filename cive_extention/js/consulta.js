  ...
  /* NUEVO: acciones por sección */
    ...
    function clearSectionEye(container, sectionId, eye) {
        ...
        sectionDef.fields.forEach((field) => {
            const el = container.querySelector(`[name="${sectionId}-${field.id}-${eye}"]`);
            if (el) el.value = '';
        });
    }
    ...
    function copySection(container, sectionId, fromEye, toEye) {
        ...
        if (!sectionDef) return;
        sectionDef.fields.forEach((field) => {
            const fromEl = container.querySelector(`[name="${sectionId}-${field.id}-${fromEye}"]`);
            const toEl = container.querySelector(`[name="${sectionId}-${field.id}-${toEye}"]`);
            if (toEl && fromEl) {
    ...
    function copyField(container, sectionId, fieldId, fromEye, toEye) {
        const fromEl = container.querySelector(`[name="${sectionId}-${fieldId}-${fromEye}"]`);
        const toEl = container.querySelector(`[name="${sectionId}-${fieldId}-${toEye}"]`);
        if (toEl && fromEl) {
            toEl.value = fromEl.value;
        }
    }
...
function realizarSecuenciaDeAcciones(item) {
    return hacerClickEnPresuntivo(
        ...
        .then(() =>
            hacerClickEnSelect2("#select2-consultas-fisico-0-tipoexamen_id-container")
        )
        .then(() =>
            establecerBusqueda(
