...
async function obtenerProcedimientosDesdeApi(afiliacion) {
    ...
    const clave = afiliacionNormalizada && afiliacionNormalizada !== "" ? afiliacionNormalizada : "__general__";
    ...
    procedimientosCache.set(clave, {
        data: procedimientos, timestamp: Date.now(),
    });
...
function detectarHcNumber() {
    ...
    const selectors = ["#docsolicitudprocedimientosdoctorsearch-hcnumber", 'input[name="DocSolicitudProcedimientosDoctorSearch[hcNumber]"]', "#historiaclinica-historiaclinica", 'input[name="HistoriaClinica[historiaClinica]"]',];
...
function renderCirugias(lista) {
    ...
    const getFechaOrden = (item) => {
        const raw = item?.fecha || item?.created_at || item?.createdAt || item?.fecha_creacion || item?.fecha_creado;
    ...
    const estadoConfig = (estado) => {
        ...
        switch (norm) {
            ...
            case "APROBADA":
                return {
                    cls: "badge-ok", icon: "✅", label: "Aprobada",
                };
            case "RECHAZADA":
                return {
                    cls: "badge-error", icon: "❌", label: "Rechazada",
                };
            case "EN_PROCESO":
                return {
                    cls: "badge-info", icon: "🔄", label: "En proceso",
                };
            default:
                return {
                    cls: "badge-info", icon: "ℹ️", label: estado || "Sin estado",
                };
        }
    };
...
>>>>>>> theirs
    ...
    listaOrdenada.forEach((item, idx) => {
        ...
        const lenteInfo = item?.lente_nombre || item?.lente_id || item?.lente_poder || item?.incision ? `
<<<<<<< ours
<<<<<<< ours
=======
<<<<<<< ours
...
>>>>>>> theirs
        ...
        const ojoTag = item?.ojo ? `<span class="timeline-chip chip-ojo">👁️ ${item.ojo}</span>` : "";
        const afiliacionTag = item?.afiliacion ? `<span class="timeline-chip chip-afiliacion">🏥 ${item.afiliacion}</span>` : "";
        const prioridadTag = item?.prioridad ? `<span class="timeline-chip chip-prioridad">⚡ ${item.prioridad}</span>` : "";

        const observacion = item?.observacion && item.observacion.trim().length ? item.observacion.trim() : "";
...
>>>>>>> theirs
    ...
    setSectionState("cirugia", {
        type: "ready", containerId: "contenedorCirugias",
    });
}
...
>>>>>>> theirs
    ...
    if (!hc) {
        setSectionState("cirugia", {
            type: "error", message: "Ingresa un HC para consultar las solicitudes.", containerId: "contenedorCirugias",
        });
        return;
<<<<<<< ours
<<<<<<< ours
...
async function obtenerEstadoCirugias(hc) {
    ...
    if (isExtensionContextActive()) {
        const resp = await safeSendMessage({
            action: "solicitudesEstado", hcNumber: hc, pageOrigin: (window.location && window.location.origin) || null,
        });
...
function normalizeSolicitudPayload(payload) {
    ...
    if (cleaned.fecha && typeof cleaned.fecha === "string") {
        let f = cleaned.fecha;
    ...
    if (cleaned.lente_id !== undefined) {
        const n = Number(cleaned.lente_id);
        if (!Number.isNaN(n)) cleaned.lente_id = n;
    }
...
async function actualizarSolicitud(id, payload) {
    ...
    const normalizedPayload = normalizeSolicitudPayload(payload);
    const resp = await safeSendMessage({
        action: "solicitudActualizar", id, payload: normalizedPayload,
    });
    if (resp && resp.success !== false) {
        return resp.data ?? resp;
    }
    if (resp?.debug?.status === 500 && (!resp?.debug?.body || String(resp.debug.body).trim() === "")) {
        console.warn("solicitudActualizar devolvió 500 sin body; continuando en modo tolerante.", resp,);
        return {__softError: true, __debug: resp?.debug};
    }
    console.error("Respuesta error solicitudActualizar:", resp);
    const debugStatus = resp?.debug?.status ? `HTTP ${resp.debug.status}` : "";
    const debugBody = resp?.debug?.body ? `| ${resp.debug.body}` : "";
    const base = resp?.error || "No se pudo actualizar la solicitud";
    throw new Error([base, debugStatus, debugBody].filter(Boolean).join(" "));
}
...
async function marcarChecklistApto(item) {
    ...
    try {
        const origin = (window.location && window.location.origin) || "https://asistentecive.consulmed.me";
        const candidates = [`${origin.replace(/\/+$/, "")}/solicitudes/actualizar-estado`, "https://asistentecive.consulmed.me/solicitudes/actualizar-estado",];
        ...
        if (resp?.success) {
            ...
            if (idx >= 0) {
                uiState.cirugias[idx] = {
                    ...
                    checklist_progress: resp.checklist_progress || uiState.cirugias[idx].checklist_progress,
                };
            }
        } else if (lastError) {
            throw lastError;
...
<<<<<<< ours
  ...
  if (
    resp?.debug?.status === 500 &&
    (!resp?.debug?.body || String(resp.debug.body).trim() === "")
  ) {
    console.warn(
      "solicitudActualizar devolvió 500 sin body; continuando en modo tolerante.",
      resp,
    );
    return { __softError: true, __debug: resp?.debug };
  }
...
async function marcarChecklistApto(item) {
  ...
  try {
    ...
    if (resp?.success) {
      const idx = uiState.cirugias.findIndex(
        (c) => String(c.id) === String(solicitudId),
      );
      if (idx >= 0) {
        uiState.cirugias[idx] = {
          ...uiState.cirugias[idx],
...
<<<<<<< ours
  return (
    ...
    document.querySelector(
      'textarea[name="DocSolicitudProcedimientos[observacion_consulta]"]',
    )
  );
}

async function appendPlanQuirurgicoEnTextarea(payload) {
  let textarea = findPlanTextarea();
  if (!textarea) {
    await new Promise((resolve) => setTimeout(resolve, 300));
    textarea = findPlanTextarea();
  }
  if (!textarea) {
...
=======
>>>>>>> theirs
    return (document.getElementById("docsolicitudprocedimientos-observacion_consulta",) || document.querySelector('textarea[name="DocSolicitudProcedimientos[observacion_consulta]"]',));
}
...
async function appendPlanQuirurgicoEnTextarea(payload) {
    ...
    if (payload?.lente_nombre || payload?.lente_id) {
        detalles.push(`Lente: ${payload.lente_nombre || "N/D"}${payload.lente_id ? " (" + payload.lente_id + ")" : ""}`,);
    }
    if (payload?.lente_poder) {
        detalles.push(`Poder: ${payload.lente_poder}`);
    }
...
>>>>>>> theirs
        ...
        try {
            setSectionState("cirugia", {
                type: "loading", message: "Guardando cambios...", containerId: "contenedorCirugias",
            });
            const updateResp = await actualizarSolicitud(item.id, payload);
            ...
            await appendPlanQuirurgicoEnTextarea(payload);
            if (updateResp?.__softError) {
                Swal.fire("Guardado con advertencia", "El servidor devolvió 500 sin cuerpo, pero se aplicaron los cambios localmente.", "warning",);
            } else {
...
function notificarCirugiasPrevias(lista, hc) {
    ...
    const resumen = lista
        .slice(0, 3)
        .map((c) => `${c.procedimiento || "Procedimiento"} · ${c.estado || "Estado N/D"}`,)
        .join("\n");
    ...
    try {
      ...
      if (updateResp?.__softError) {
        Swal.fire(
          ...
          "El servidor devolvió 500 sin cuerpo, pero se aplicaron los cambios localmente.",
          "warning",
        );
      } else {
        Swal.fire("Guardado", "Solicitud actualizada en MedForge", "success");
...
async function cargarExamenes() {
    ...
    setSectionState("examenes", {
        type: "loading", message: "Cargando exámenes clínicos...", containerId: contenedorId,
    });
<<<<<<< ours
    ...
    try {
        ...
        if (!examenes.length) {
            ...
            setSectionState("examenes", {
                type: "empty", message: "No se encontraron exámenes configurados.", containerId: contenedorId,
            });
            return;
        }
...
async function cargarRecetas() {
    ...
    setSectionState("recetas", {
        type: "loading", message: "Sincronizando recetas con Medforge...", containerId: contenedorId,
    });
    ...
    try {
        ...
        if (!recetas.length) {
            document.getElementById(contenedorId).innerHTML = "";
            setSectionState("recetas", {
                type: "empty", message: "No hay recetas disponibles para este paciente.", containerId: contenedorId,
=======
...
function renderRecetasPorCategoria({query = ""} = {}) {
    ...
    if (!recetas.length) {
        document.getElementById(contenedorId).innerHTML = "";
        setSectionState("recetas", {
            type: "empty", message: "Aún no hay recetas configuradas.", containerId: contenedorId,
        });
        return;
    }
<<<<<<< ours
...
>>>>>>> theirs
        if (!resultados.length) {
            document.getElementById(contenedorId).innerHTML = "";
            setSectionState("recetas", {
                type: "empty", message: `No se encontraron recetas para "${query}".`, containerId: contenedorId,
            });
            return;
        }
...
>>>>>>> theirs
    ...
    if (!recetas.length) {
        document.getElementById(contenedorId).innerHTML = "";
        setSectionState("recetas", {
            type: "empty", message: `No hay recetas guardadas en ${categoria}.`, containerId: contenedorId,
        });
        return;
    }
...
>>>>>>> theirs
    const contenedorId = "contenedorProtocolos";
    setSectionState("protocolos", {
        type: "loading", message: "Buscando protocolos disponibles...", containerId: contenedorId,
    });
<<<<<<< ours
<<<<<<< ours
...
>>>>>>> theirs
        ...
        try {
            ...
            if (fallbackProcedimientos.length) {
                ...
                renderProtocolosPorCategoria();
                setSectionState("protocolos", {
                    type: "ready", containerId: contenedorId,
                });
...
function renderProtocolosPorCategoria({query = ""} = {}) {
    ...
    if (!procedimientos.length) {
        document.getElementById(contenedorId).innerHTML = "";
        setSectionState("protocolos", {
            type: "empty", message: "Sin protocolos disponibles.", containerId: contenedorId,
        });
        return;
    }
<<<<<<< ours
...
>>>>>>> theirs
        if (!resultados.length) {
            document.getElementById(contenedorId).innerHTML = "";
            setSectionState("protocolos", {
                type: "empty", message: `No hay protocolos que coincidan con "${query}".`, containerId: contenedorId,
            });
            return;
        }
...
function renderProcedimientos(lista, {query = ""} = {}) {
    ...
    if (!lista.length) {
        ...
        setSectionState("procedimientos", {
            type: "empty", message: "Selecciona una categoría para ver sus procedimientos.", containerId: contenedorId,
        });
        return;
    }
    ...
    if (query) {
        ...
        if (!procedimientos.length) {
            document.getElementById(contenedorId).innerHTML = "";
            setSectionState("procedimientos", {
                type: "empty", message: `No se encontraron procedimientos para "${query}".`, containerId: contenedorId,
            });
            return;
        }
    }
...
function ejecutarReceta(id) {
    cargarJSON(safeGetURL("data/recetas.json"))
        ...
        .catch((error) => {
            ...
            setSectionState("recetas", {
                type: "error", message: "No se pudo insertar la receta seleccionada.", containerId: "contenedorRecetas",
            });
        });
}
