...
<script>
    function configure_ai_editor(editor) {
        const requestEnhancement = function(type) {
            const selectedHtml = editor.selection.getContent({ format: 'html' }).trim();

            if (!selectedHtml) {
                alert_float('warning', app.lang.something_went_wrong);
            ...
            $.ajax({
                url: admin_url + 'ai/text_enhancement/' + type,
                method: 'POST',
                dataType: 'json',
                data: {
                    text: selectedHtml,
                ...
                const message = xhr.responseJSON && xhr.responseJSON.error ? xhr.responseJSON.error : app.lang.something_went_wrong;
