...
class IplHelper
{
    public static function calcularTotalFactura(array $datosPaciente, IplPlanificadorController $iplPlanificadorController): float
    {
        $total = 0;
        ...
        if (!empty($datosPaciente['procedimientos'][0])) {
            ...
            $precioReal = $codigo ? $iplPlanificadorController->obtenerValorAnestesia($codigo) : null;
    ...
    public static function obtenerConsolidadoFiltrado(
        array                     $facturas,
        array                     $filtros,
        IplPlanificadorController $iplPlanificadorController,
    ...
    ): array
    {
        ...
        foreach ($facturas as $factura) {
            ...
            $datosPaciente = $iplPlanificadorController->obtenerDatos($factura['form_id']);
            ...
            $total = IplHelper::calcularTotalFactura($datosPaciente, $iplPlanificadorController);
    ...
    public static function filtrarPacientes(array $pacientes, array &$pacientesCache, array &$datosCache, $pacienteController, $iplPlanificadorController, string $apellidoFiltro): array
    {
        return array_filter($pacientes, function ($p) use (&$pacientesCache, &$datosCache, $pacienteController, $iplPlanificadorController, $apellidoFiltro) {
            $hc = $p['hc_number'];
            $fid = $p['form_id'];
            ...
            if (!isset($datosCache[$fid])) {
                $datosCache[$fid] = $iplPlanificadorController->obtenerDatos($fid);
            }
