                            ...
                            <h2 class="accordion-header d-flex justify-content-between align-items-center px-3"
                                ...
                                <div class="d-flex flex-grow-1 align-items-center">
                                    <button class="accordion-button collapsed flex-grow-1 text-start" type="button"
                                        ...
                                        <?= htmlspecialchars($categoria, ENT_QUOTES, 'UTF-8') ?> (<?= count($procedimientos) ?>)
                                    </button>
                                </div>
                                <div class="ms-3">
                                    <a href="/protocolos/crear?categoria=<?= urlencode($categoria) ?>"
                                       class="btn btn-sm btn-outline-primary">
                                        <i class="mdi mdi-plus-circle-outline me-5"></i> Nuevo protocolo en esta categoría
                                    </a>
                                </div>
                            </h2>
                                        ...
                                        <div class="d-flex align-items-center mb-30 border-bottom pb-15">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item" href="/protocolos/editar?id=<?= urlencode($procedimiento['id']) ?>">Editar</a>
                                                    <a class="dropdown-item" href="/protocolos/editar?duplicar=<?= urlencode($procedimiento['id']) ?>">Duplicar</a>
                                                    <form method="POST" action="/protocolos/eliminar" style="display:inline;"
                                                          onsubmit="return confirm('¿Estás seguro de que deseas eliminar este protocolo?');">
                                                        <input type="hidden" name="id" value="<?= htmlspecialchars($procedimiento['id'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
