...
class CirugiaService
{
    ...
    public function guardarDesdeApi(array $data): array
    {
        ...
        $this->ensurePatientDataForProtocol($data);
        if ($this->lastError !== null) {
    ...
    private function ensurePatientDataForProtocol(array $data): void
    {
        if (!$this->tableExists('patient_data')) {
            $this->lastError = 'No existe la tabla patient_data para vincular el protocolo.';
            return;
        }

        $hcNumber = trim((string) ($data['hc_number'] ?? ''));
        if ($hcNumber === '') {
            $this->lastError = 'No se puede crear paciente sin hc_number.';
            return;
        }
        ...
        $optionalFields = [
            'fecha_nacimiento' => $this->normalizeProtocolDate($data['fechaNacimiento'] ?? $data['fecha_nacimiento'] ?? null),
            'sexo' => $this->cleanProtocolPatientValue($data['sexo'] ?? null),
            'celular' => $this->cleanProtocolPatientValue($data['celular'] ?? null),
            'ciudad' => $this->cleanProtocolPatientValue($data['ciudad'] ?? null),
            'afiliacion' => $this->cleanProtocolPatientValue($data['afiliacion'] ?? null),
        ];

        foreach ($optionalFields as $column => $value) {
            if ($value !== null) {
                $insertFields[$column] = $value;
            }
        }

        $auditType = 'api';
        $auditIdentifier = 'api:/protocolos/guardar.php';
        if ($this->columnExists('patient_data', 'created_by_type')) {
            $insertFields['created_by_type'] = $auditType;
        }
        if ($this->columnExists('patient_data', 'created_by_identifier')) {
            $insertFields['created_by_identifier'] = $auditIdentifier;
        }
        if ($this->columnExists('patient_data', 'updated_by_type')) {
            $insertFields['updated_by_type'] = $auditType;
        }
        if ($this->columnExists('patient_data', 'updated_by_identifier')) {
            $insertFields['updated_by_identifier'] = $auditIdentifier;
        }

        $insertFields = array_filter(
            $insertFields,
            fn($_value, string $column): bool => $this->columnExists('patient_data', $column),
            ARRAY_FILTER_USE_BOTH
        );

        if (!isset($insertFields['hc_number'])) {
            $this->lastError = 'patient_data no tiene columna hc_number.';
            return;
        }
        ...
        $updates = [];
        foreach (array_keys($optionalFields) as $column) {
            if (!array_key_exists($column, $insertFields)) {
                continue;
            }
        ...
        if ($this->columnExists('patient_data', 'updated_at')) {
            $updates[] = 'updated_at = CURRENT_TIMESTAMP';
        }
        if ($this->columnExists('patient_data', 'updated_by_type') && isset($insertFields['updated_by_type'])) {
            $updates[] = 'updated_by_type = VALUES(updated_by_type)';
        }
        if ($this->columnExists('patient_data', 'updated_by_identifier') && isset($insertFields['updated_by_identifier'])) {
            $updates[] = 'updated_by_identifier = VALUES(updated_by_identifier)';
        }

        if ($updates === []) {
            $updates[] = 'hc_number = VALUES(hc_number)';
        }
        ...
        try {
            ...
            $this->lastError = 'No se pudo crear o actualizar patient_data para el protocolo: ' . $exception->getMessage();
        }
    }

    private function cleanProtocolPatientValue(mixed $value): ?string
    {
        ...
        if ($value === '' || strtoupper($value) === 'SELECCIONE') {
            return null;
        }
    ...
    private function normalizeProtocolDate(mixed $value): ?string
    {
        $value = $this->cleanProtocolPatientValue($value);
        if ($value === null) {
            return null;
        }
