...
if (!defined('STDERR')) {
    /**
     * En entornos donde PHP se ejecuta como CGI/FPM (por ejemplo hosting compartido),
     * la constante STDERR no está definida. En ese caso escribimos directamente en
     * php://stderr para mantener los mensajes de error visibles.
     */
...
{
    $stream = defined('STDERR') && is_resource(STDERR) ? STDERR : fopen('php://stderr', 'w');
...
function connect(): PDO
{
    try {
        ...
        write_stderr("[error] No fue posible conectar a la base de datos: {$e->getMessage()}" . PHP_EOL);
        write_stderr("        Verifica las variables de entorno DB_HOST, DB_NAME, DB_USER y DB_PASSWORD." . PHP_EOL);
        exit(1);
    }
}
    ...
    if (!empty($options['scenario'])) {
        ...
        foreach ($requested as $scenarioName) {
            ...
            if (!isset($allScenarios[$scenarioName])) {
                write_stderr("[error] Escenario desconocido: {$scenarioName}" . PHP_EOL);
                write_stderr("        Ejecuta con --list para ver las opciones disponibles." . PHP_EOL);
                exit(1);
            }
    ...
    foreach ($selected as $name => $scenario) {
        ...
        foreach ($paramsSets as $index => $params) {
            ...
            try {
                ...
                write_stderr("[error] No se pudo ejecutar EXPLAIN para {$label}: {$e->getMessage()}" . PHP_EOL);
            }
        }
    }
}
