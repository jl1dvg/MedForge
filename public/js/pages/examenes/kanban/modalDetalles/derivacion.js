    ...
    if (diagnosticos.length) {
        const items = diagnosticos
            .map((dx) => {
                ...
                const descripcion = escapeHtml(dx?.descripcion || dx?.diagnostico || "");
                const lateralidad = dx?.lateralidad ? ` (${escapeHtml(dx.lateralidad)})` : "";
...
export async function asegurarPreseleccionDerivacion({ hc, formId, examenId }) {
    if (!hc || !formId) {
        return null;
    }
            ...
            list.querySelectorAll("[data-derivacion-option]").forEach((btn) => {
                btn.addEventListener("click", () => {
                    ...
                    list.querySelectorAll("[data-derivacion-option]").forEach((item) => {
                        item.classList.toggle("border-primary", item === btn);
                    });
                });
            });
        },
