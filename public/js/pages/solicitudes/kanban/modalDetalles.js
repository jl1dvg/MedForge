import { getKanbanConfig, getTableBodySelector, getDataStore, getEstadosMeta } from './config.js';
...
const SLA_META = {
    en_rango: { label: 'En rango', className: 'text-success fw-semibold', icon: 'mdi-check-circle-outline' },
    advertencia: { label: 'Seguimiento 72h', className: 'text-warning fw-semibold', icon: 'mdi-timer-sand' },
    critico: { label: 'Crítico 24h', className: 'text-danger fw-semibold', icon: 'mdi-alert-octagon' },
    vencido: { label: 'SLA vencido', className: 'text-dark fw-semibold', icon: 'mdi-alert' },
    sin_fecha: { label: 'Sin programación', className: 'text-muted', icon: 'mdi-calendar-remove' },
    cerrado: { label: 'Cerrado', className: 'text-muted', icon: 'mdi-lock-outline' },
};

const ALERT_TEMPLATES = [
    { field: 'alert_reprogramacion', label: 'Reprogramar', icon: 'mdi-calendar-alert', className: 'badge bg-danger text-white' },
    { field: 'alert_pendiente_consentimiento', label: 'Consentimiento', icon: 'mdi-shield-alert', className: 'badge bg-warning text-dark' },
];
...
function getEstadoBadge(estado) {
    ...
    const textClass = STATUS_BADGE_TEXT_DARK.has(color) ? 'text-dark' : 'text-white';
...
function buildSlaInfo(solicitud = {}) {
    ...
    const hours = formatHoursRemaining(typeof solicitud.sla_hours_remaining === 'number'
    ...
    const detail = detailParts.length ? detailParts.join(' · ') : 'Sin referencia SLA';
...
function buildPrioridadInfo(solicitud = {}) {
    ...
    const prioridad = solicitud.prioridad || solicitud.prioridad_automatica || 'Normal';
    return {
        label: prioridad,
        helper: origenManual ? 'Asignada manualmente' : 'Regla automática',
        className: origenManual ? 'text-primary fw-semibold' : 'text-success fw-semibold',
    };
}

function buildStatsHtml(solicitud = {}) {
    const stats = [
        { label: 'Notas', value: Number.parseInt(solicitud.crm_total_notas ?? 0, 10), icon: 'mdi-note-text-outline' },
        { label: 'Adjuntos', value: Number.parseInt(solicitud.crm_total_adjuntos ?? 0, 10), icon: 'mdi-paperclip' },
        {
            label: 'Tareas abiertas',
            value: `${Number.parseInt(solicitud.crm_tareas_pendientes ?? 0, 10)}/${Number.parseInt(solicitud.crm_tareas_total ?? 0, 10)}`,
    ...
    return stats.map(stat => `
        <div class="prefactura-state-stat">
            ...
            <span class="fw-semibold">
                ${stat.icon ? `<i class="mdi ${escapeHtml(stat.icon)} me-1"></i>` : ''}
...
function buildAlertsHtml(solicitud = {}) {
    const alerts = ALERT_TEMPLATES
        .filter(template => Boolean(solicitud[template.field]))
        .map(template => `
            <span class="${escapeHtml(template.className)}">
                <i class="mdi ${escapeHtml(template.icon)} me-1"></i>${escapeHtml(template.label)}
            </span>
        `);
...
function renderGridItem(label, value, helper = '', valueClass = '') {
    ...
    const helperHtml = helper ? `<span class="text-muted small d-block mt-1">${escapeHtml(helper)}</span>` : '';
    const className = valueClass ? ` ${valueClass}` : '';
    return `
        <div class="prefactura-state-grid-item">
            <small>${escapeHtml(label)}</small>
            <strong class="prefactura-state-value${className}">${escapeHtml(value)}</strong>
            ${helperHtml}
        </div>
    `;
}
...
function syncQuickColumnVisibility() {
    ...
    const summaryVisible = summary ? !summary.classList.contains('d-none') : false;
...
function renderEstadoContext(solicitudId) {
    ...
    if (!solicitud) {
        container.innerHTML = '<div class="alert alert-light border mb-0">No se encontró información del estado seleccionado.</div>';
    ...
    const turnoLabel = turno ? `Turno #${turno}` : 'Sin turno asignado';
    const secuencia = solicitud.secuencia ? `Secuencia ${solicitud.secuencia}` : '';
    ...
    const proximoVencimiento = solicitud.crm_proximo_vencimiento
        ? formatIsoDate(solicitud.crm_proximo_vencimiento, 'Sin vencimiento', 'DD-MM-YYYY')
        : 'Sin vencimiento';

    const gridItems = [
        renderGridItem('Prioridad', prioridadInfo.label, prioridadInfo.helper, prioridadInfo.className),
        renderGridItem('Seguimiento SLA', slaInfo.label, slaInfo.detail, slaInfo.className),
        ...
        renderGridItem('Turno', turnoLabel, secuencia || estadoBadge.label),
        ...
        <div class="prefactura-state-card">
            <div class="prefactura-state-header">
                <div>
                    ...
                    <span class="${escapeHtml(estadoBadge.badgeClass)}">${escapeHtml(estadoBadge.label)}</span>
                </div>
                <div>
                    <p class="text-muted mb-1">Etapa CRM</p>
                    <span class="badge bg-light text-dark border">${escapeHtml(pipelineStage)}</span>
                </div>
            </div>
...
function renderPatientSummaryFallback(solicitudId) {
    ...
    const turno = formatTurno(solicitud.turno);
    const doctor = solicitud.doctor || solicitud.crm_responsable_nombre || 'Sin doctor';
    const procedimiento = solicitud.procedimiento || 'Sin procedimiento';
    const afiliacion = solicitud.afiliacion || solicitud.aseguradora || 'Sin afiliación';
    const hcNumber = solicitud.hc_number || '—';

    container.innerHTML = `
        <div class="alert alert-primary text-center fw-bold mb-0 prefactura-patient-alert">
            <div>🧑 Paciente: ${escapeHtml(solicitud.full_name || 'Sin nombre')}</div>
            <small class="d-block text-uppercase mt-1">${escapeHtml(`HC ${hcNumber}`)}</small>
            <small class="d-block">${escapeHtml(doctor)}</small>
            <small class="d-block text-muted">${escapeHtml(procedimiento)}</small>
            <small class="d-block text-muted">${escapeHtml(afiliacion)}</small>
            ${turno ? `<span class="badge bg-light text-primary mt-2">Turno #${escapeHtml(turno)}</span>` : ''}
        </div>
    `;
    container.classList.remove('d-none');
    syncQuickColumnVisibility();
...
function relocatePatientAlert(solicitudId) {
    ...
    const patientAlert = alerts.find(element => PATIENT_ALERT_TEXT.test(element.textContent || ''));
