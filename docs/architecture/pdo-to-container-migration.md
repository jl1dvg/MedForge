...
| 5 | **Cirugías** — ✅ Fase 0+1 completa | `CirugiaService`, `ProtocolosTemplateReadService`, `ProtocolosTemplateWriteService`, `CirugiasDerivacionService`, `CirugiasDashboardService` | Media-Alta | Medio-Alto | `AfiliacionDimensionService` (paso 1, completo) | **Sí, prioritario** — diseño de Fase 2 ya documentado en [`cirugias-dashboard-service-fase2-design.md`](./cirugias-dashboard-service-fase2-design.md) (6 dominios identificados, Repositories propuestos, orden de extracción). Extracción **no iniciada** — pendiente de aprobación explícita. | Migrado fuera de orden (antes que CRM) por decisión explícita: validar el patrón sobre el caso más complejo del sistema antes de replicarlo. Los 5 servicios (incluye `CirugiasDerivacionService`, encontrado durante la ejecución — no estaba en la auditoría original por tener constructor multilínea) resueltos por el container, cero cambios de SQL/comportamiento verificados con diff aislado + suite de tests. |
