...
class IssfaIndividualExcelBuilder
{
    ...
    public function build(array $datos, string $formId): Spreadsheet
    {
        ...
        $cirujano = $datos['protocoloExtendido']['cirujano_1'] ?? '';
        $edadCalculada = $formDetails['edad'] ?? '';

        $derivacion   = $this->billingService->obtenerDerivacionPorFormId($formId);
        $diagnosticoStr = $derivacion['diagnostico'] ?? '';
        $cie101 = '';
        $cie102 = '';
        if ($diagnosticoStr !== '') {
            $codigosUnicos = [];
            foreach (explode(';', $diagnosticoStr) as $dx) {
                $dx = trim($dx);
                if ($dx === '') {
                    continue;
                }
        ...
        $contexto = [
            ...
            'edad'          => isset($pacienteInfo['fecha_nacimiento'])
                ? date_diff(date_create((string) $pacienteInfo['fecha_nacimiento']), date_create((string) $fechaISO))->y
                : null,
        ];

        $codigoAnestesia = $datos['procedimientos'][0]['proc_codigo'] ?? '';
        $precioReal      = $codigoAnestesia
        ...
        $procHeaders = [
            'B' => 'FECHA', 'C' => 'CODIGO (CPT/TARIFARIO)', 'D' => 'DETALLE/DESCRIPCION',
            'E' => 'COSTO UNITARIO', 'I' => 'CANTIDAD', 'J' => 'COSTO TOTAL',
        ];
        $sheet->mergeCells("E{$row}:H{$row}");
        $sheet->getRowDimension($row)->setRowHeight(35);
        foreach ($procHeaders as $col => $label) {
        ...
        foreach ($datos['anestesia'] as $a) {
            if (in_array($a['codigo'], ['99149AA', '99150AA'], true)) {
        ...
        if ($anestesiaEspecialYaCobrada) {
            foreach ($datos['anestesia'] as $a) {
                ...
                $sheet->setCellValue("B{$row}", $formDetails['fecha_inicio'] ?? '');
                $sheet->setCellValue("C{$row}", $a['codigo']);
                $sheet->setCellValue("D{$row}", $a['nombre']);
            ...
            if (!empty($datos['procedimientos'][0])) {
                ...
                $sheet->setCellValue("F{$row}", $precio);
                $sheet->setCellValue("G{$row}", '');
                $sheet->setCellValue("J{$row}", $precioReal);
                $sheet->setCellValue("K{$row}", 'ANESTESIOLOGO');
                foreach (['B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'] as $col) {
                    $sheet->getStyle("{$col}{$row}")->getBorders()->getAllBorders()->setBorderStyle('thin');
                }
    ...
    private function buildMedicinaSection(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $sheet, int $row, array $datos, array $formDetails): array
    {
        ...
        foreach ($datos['oxigeno'] as $o) {
            $codigo       = ($o['codigo'] === '911111') ? '241208' : ($o['codigo'] ?? '');
