...
namespace App\Modules\Usuarios\Support;

use Illuminate\Http\UploadedFile;

final class UserMediaValidator
{
    ...
    public function validate(?UploadedFile $file): array
    {
        ...
        if (!$file->isValid()) {
            return $this->result($this->uploadErrorMessage($file->getError()));
        }
     ...
     */
    public function destinationFor(string $type, string $filename, string $publicRoot): array
    {
        $directory = self::TYPE_DIRECTORIES[$type] ?? self::TYPE_DIRECTORIES[self::TYPE_SEAL];
        $relative = '/uploads/users/' . $directory;
        $absoluteDir = rtrim($publicRoot, '/') . $relative;
    ...
    private function resolveDimensions(string $path, string $mime): array
    {
        if ($mime === 'image/svg+xml') {
            ...
            return [
                $this->extractSvgSize($svg, 'width'),
                $this->extractSvgSize($svg, 'height'),
            ];
        }
