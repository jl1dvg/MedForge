...
class DoctoresService
{
    ...
    public function allDoctors(): array
    {
        $sql = <<<'SQL'
            ...
            WHERE u.especialidad = 'Cirujano Oftalmólogo'
     ...
     */
    public function findDoctor(int $id): ?array
    {
        $sql = <<<'SQL'
            SELECT
                u.id,
     ...
     */
    public function buildDoctorInsights(array $doctor, ?string $selectedDate): array
    {
     ...
     * @return array<string, mixed>
     */
    public function getDoctorCardPerformanceSummary(array $doctor): array
    {
    ...
    /**
     * @param array<string, mixed> $doctor
     * @return array{selectedDate: ?string, selectedLabel: ?string, appointments: array<int, array<string, mixed>>, days: array<int, array<string, mixed>>}
     */
    public function buildAppointmentsJsonPayload(array $doctor, ?string $requestedDate): array
    {
        $schedule = $this->buildAppointmentsSchedule($doctor, $requestedDate);

        return [
            'selectedDate' => $schedule['selectedDate'],
            'selectedLabel' => $schedule['selectedLabel'],
    ...
    // -------------------------------------------------------------------------
    // Private business logic (ported from legacy DoctoresController)
    // -------------------------------------------------------------------------
     ...
     */
    private function buildTodayPatients(array $doctor): array
    {
     ...
     */
    private function buildAppointmentsSchedule(array $doctor, ?string $requestedDate): array
    {
     ...
     * @return array<int, array<string, mixed>>
     */
    private function buildActivityStats(array $doctor): array
    {
     ...
     * @return array<int, array<string, mixed>>
     */
    private function buildCareProgress(array $doctor): array
    {
        $context = $this->getDoctorPerformanceContext($doctor);
        $agenda = $context['agenda_30'] ?? [];
        $surgeries = $context['surgeries_90'] ?? [];
     ...
     * @return array<int, array<string, string>>
     */
    private function buildMilestones(array $doctor): array
    {
        $context = $this->getDoctorPerformanceContext($doctor);
     ...
     */
    private function buildBiographyParagraphs(array $doctor): array
    {
        $context = $this->getDoctorPerformanceContext($doctor);
     ...
     * @return array<string, mixed>
     */
    private function buildAvailabilitySummary(array $doctor): array
    {
        $context = $this->getDoctorPerformanceContext($doctor);
     ...
     * @return array<int, array<string, string>>
     */
    private function buildSupportChannels(array $doctor): array
    {
        $context = $this->getDoctorPerformanceContext($doctor);
     ...
     * @return array<int, array<string, string>>
     */
    private function buildResearchHighlights(array $doctor): array
    {
        $context = $this->getDoctorPerformanceContext($doctor);
    ...
    {
        $empty = [
            'total' => 0, 'unique_patients' => 0, 'completed' => 0,
            'lost' => 0, 'rescheduled' => 0, 'first_hour' => '',
            'last_hour' => '', 'weekdays' => '', 'latest_date' => '',
        ];
        $lookupValues = $this->resolveDoctorLookupValues($doctor);
        if ($lookupValues === []) {
            return $empty;
    ...
    // -------------------------------------------------------------------------
    // DoctorModel logic (mapDoctorRow + helpers)
    // -------------------------------------------------------------------------
     ...
     * @param array<int, string> $lookupValues
     */
    private function buildMatchClause(array $lookupValues, string $expression, bool $useLike, string $paramPrefix): array
    {
        $conditions = [];
        $params = [];
    ...
    private function formatTrend(int $value): array
    {
        ...
        return ['value' => $formatted, 'direction' => $direction];
    }
    ...
    {
        $map = ['1' => 'Dom', '2' => 'Lun', '3' => 'Mar', '4' => 'Mié', '5' => 'Jue', '6' => 'Vie', '7' => 'Sáb'];
        $labels = [];
    ...
    /**
     * @param array<string, mixed> $doctor
     * Resolve a profile photo path to a URL (mirrors legacy format_profile_photo_url).
     */
    public static function resolveProfilePhotoUrl(?string $path): ?string
    {
        if ($path === null || $path === '') {
            return null;
        }
