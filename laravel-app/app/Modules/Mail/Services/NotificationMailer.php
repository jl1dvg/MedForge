...
class NotificationMailer
{
     ...
     * @return array<string, mixed>
     */
    private function getSettingOptions(array $keys): array
    {
        $keys = array_values(array_unique(array_filter(array_map(static fn($key): string => trim((string)$key), $keys))));
        if ($keys === []) {
            return [];
        }

        $tables = [
            ['table' => 'settings', 'key' => 'option_name', 'value' => 'option_value'],
            ['table' => 'settings', 'key' => 'setting_key', 'value' => 'setting_value'],
            ['table' => 'settings', 'key' => 'key', 'value' => 'value'],
            ['table' => 'configuraciones', 'key' => 'clave', 'value' => 'valor'],
        ];

        foreach ($tables as $tableConfig) {
            $table = $tableConfig['table'];
            $keyColumn = $tableConfig['key'];
            $valueColumn = $tableConfig['value'];

            if (!$this->tableExists($table) || !$this->columnExists($table, $keyColumn) || !$this->columnExists($table, $valueColumn)) {
                continue;
            }

            $rows = DB::table($table)
                ->whereIn($keyColumn, $keys)
                ->get([$keyColumn . ' as option_key', $valueColumn . ' as option_value']);
            ...
            foreach ($rows as $row) {
                ...
                $options[$optionKey] = $row->option_value ?? null;
            }
