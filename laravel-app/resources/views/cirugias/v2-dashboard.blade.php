...
@php
    $date_range = is_array($date_range ?? null) ? $date_range : [];
    $total_cirugias = (int) ($total_cirugias ?? 0);
    $cirugias_sin_facturar = (int) ($cirugias_sin_facturar ?? 0);
    $duracion_promedio = (string) ($duracion_promedio ?? '—');
    $estado_protocolos = is_array($estado_protocolos ?? null) ? $estado_protocolos : [];
    $cirugias_por_mes = is_array($cirugias_por_mes ?? null) ? $cirugias_por_mes : ['labels' => [], 'totals' => []];
    $top_procedimientos = is_array($top_procedimientos ?? null) ? $top_procedimientos : ['labels' => [], 'totals' => []];
    ...
    if (!function_exists('cirugiasDashboardInlineFormatV2')) {
        function cirugiasDashboardInlineFormatV2(string $text): string
        {
    ...
    if (!function_exists('cirugiasDashboardRenderMarkdownV2')) {
        function cirugiasDashboardRenderMarkdownV2(string $markdown): string
        {
            ...
            foreach ($lines as $line) {
                ...
                if (preg_match('/^(#{1,3})\s+(.+)$/', $trimmed, $m) === 1) {
                    $closeLists();
                    $level = strlen($m[1]);
                    $html[] = '<h' . $level . '>' . cirugiasDashboardInlineFormatV2($m[2]) . '</h' . $level . '>';
                    continue;
                }
                ...
                if (preg_match('/^[-*]\s+(.+)$/', $trimmed, $m) === 1) {
                    ...
                    $html[] = '<li>' . cirugiasDashboardInlineFormatV2($m[1]) . '</li>';
                    continue;
                }
                ...
                $closeLists();
                $html[] = '<p>' . cirugiasDashboardInlineFormatV2($trimmed) . '</p>';
            }
    ...
    $helpMdPath = base_path('../modules/Cirugias/docs/dashboard_quirurgico_guia.md');
    $helpMarkdown = is_file($helpMdPath)
        ? (string) file_get_contents($helpMdPath)
        : "# Guía no disponible\n\nNo se encontró el archivo de ayuda del dashboard.";
    $helpHtml = cirugiasDashboardRenderMarkdownV2($helpMarkdown);
...
@section('content')
        ...
        <div class="d-flex align-items-center">
            <div class="me-auto">
                ...
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item"><a href="/v2/cirugias">Cirugías</a></li>
    ...
    <section class="content">
        <div class="row g-3">
                ...
                <div class="box mb-3">
                    <div class="box-header with-border d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex flex-wrap gap-2 justify-content-end">
                            ...
                            <a href="/v2/cirugias" class="btn btn-outline-primary btn-sm">
                        ...
                        <form method="get" class="row g-2 align-items-end">
                            ...
                            <div class="col-sm-6 col-md-3">
                                ...
                                <select id="afiliacion" name="afiliacion" class="form-select">
                                    @foreach(($afiliacion_options ?? []) as $option)
                                        ...
                                        <option value="{{ $optionValue }}" {{ $optionValue === (string) ($afiliacion_filter ?? '') ? 'selected' : '' }}>
                                            {{ (string) ($option['label'] ?? '') }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-6 col-md-3">
                                ...
                                <select id="sede" name="sede" class="form-select">
                                    @foreach(($sede_options ?? []) as $option)
                                        @php $optionValue = (string) ($option['value'] ?? ''); @endphp
                                        <option value="{{ $optionValue }}" {{ $optionValue === (string) ($sede_filter ?? '') ? 'selected' : '' }}>
                                            {{ (string) ($option['label'] ?? '') }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12 d-flex gap-2">
                                ...
                                <a href="/v2/cirugias/dashboard" class="btn btn-outline-secondary btn-sm">
                                    <i class="mdi mdi-close-circle-outline"></i> Limpiar
                                </a>
                            </div>
                        </form>
                        <p class="text-muted fs-12 mb-0 mt-10">
                            Mostrando datos del periodo: <strong>{{ (string) ($date_range['label'] ?? '') }}</strong>
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="cirugias-kpi-grid mb-3">
                    @foreach(($kpi_cards ?? []) as $card)
                        <article class="cirugias-kpi-card">
                            <p class="cirugias-kpi-label mb-1">{{ (string) ($card['label'] ?? '') }}</p>
                            <h4 class="cirugias-kpi-value mb-1">{{ (string) ($card['value'] ?? '') }}</h4>
                            <p class="cirugias-kpi-hint mb-0">{{ (string) ($card['hint'] ?? '') }}</p>
                        </article>
                    @endforeach
                </div>

                <div class="d-flex flex-wrap gap-3 align-items-center text-muted small mb-3">
                    <span><strong>TAT promedio:</strong> {{ $tatPromedioHoras !== null ? number_format($tatPromedioHoras, 2) . ' h -> ~' . number_format($tatPromedioHoras / 24, 1) . ' dias' : '—' }}</span>
                    <span><strong>TAT mediana:</strong> {{ $tatMedianaHoras !== null ? number_format($tatMedianaHoras, 2) . ' h -> ~' . number_format($tatMedianaHoras / 24, 1) . ' dias' : '—' }}</span>
                    <span><strong>TAT P90:</strong> {{ $tatP90Horas !== null ? number_format($tatP90Horas, 2) . ' h -> ~' . number_format($tatP90Horas / 24, 1) . ' dias' : '—' }}</span>
                    <span><strong>Muestra:</strong> {{ (string) $tatMuestra }} protocolos revisados</span>
                </div>
            </div>

            <div class="col-xl-8">
                <div class="box">
                    <div class="box-header"><h4 class="box-title">Cirugías por mes</h4></div>
                    <div class="box-body"><div id="cirugias-por-mes" style="height: 300px;"></div></div>
                </div>
            </div>
            <div class="col-xl-4">
                <div class="box">
            ...
            <div class="col-xl-4">
                <div class="box">
                    <div class="box-header"><h4 class="box-title">Top procedimientos</h4></div>
                    <div class="box-body"><div id="top-procedimientos" style="height: 320px;"></div></div>
                </div>
            </div>
            <div class="col-xl-4">
                <div class="box">
                    <div class="box-header"><h4 class="box-title">Top cirujanos (realizadas)</h4></div>
                    <div class="box-body"><div id="top-cirujanos" style="height: 320px;"></div></div>
                </div>
            </div>
            <div class="col-xl-4">
                <div class="box">
                    <div class="box-header"><h4 class="box-title">Top doctores solicitantes (realizadas)</h4></div>
                    <div class="box-body"><div id="top-doctores-realizadas" style="height: 320px;"></div></div>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="cirugias-help-markdown">{!! $helpHtml !!}</div>
                </div>
