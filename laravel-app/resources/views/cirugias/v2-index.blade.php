                            ...
                            <form id="filtrosCirugias" class="row g-2 align-items-end mb-3">
                                <div class="col-sm-6 col-md-3">
                                    ...
                                    <input type="date" class="form-control" id="filtroFechaInicio" name="fecha_inicio"
                                           value="{{ $fechaInicioDefaultValue }}" data-default="{{ $fechaInicioDefaultValue }}">
                                </div>
                                <div class="col-sm-6 col-md-3">
                                    <label for="filtroFechaFin" class="form-label">Hasta</label>
                                    <input type="date" class="form-control" id="filtroFechaFin" name="fecha_fin"
                                           value="{{ $fechaFinDefaultValue }}" data-default="{{ $fechaFinDefaultValue }}">
                                </div>
                                <div class="col-sm-6 col-md-3">
        ...
        <script>
            ...
            function emitirCertificadoDescanso(formId, hcNumber) {
                ...
                if (!Number.isFinite(dias) || dias <= 0) {
                    Swal.fire({ icon: 'warning', title: 'Valor invalido', text: 'Debe ingresar un numero entero mayor a cero.' });
                    return;
                }
                const params = new URLSearchParams({ form_id: formId, hc_number: hcNumber, dias_descanso: String(dias) });
            ...
            let currentFormId, currentHcNumber;
            ...
            function renderProtocolAudit(auditoria) {
                ...
                if (!panel || !summary || !checksContainer) return;
                ...
                const alertClassMap = { ok: 'alert-success', warning: 'alert-warning', error: 'alert-danger' };
                const badgeClassMap = { ok: 'badge bg-success', warning: 'badge bg-warning text-dark', error: 'badge bg-danger' };
                const statusLabelMap = { ok: 'OK', warning: 'Advertencia', error: 'Alerta' };
                panel.style.display = checks.length > 0 ? '' : 'none';
                summary.className = `alert mb-2 ${alertClassMap[status] || 'alert-secondary'}`;
                summary.innerHTML = `<div class="d-flex justify-content-between align-items-center flex-wrap gap-2"><div><strong>Auditoría automática del protocolo</strong><div class="small">Se validó concordancia con lo proyectado y la plantilla quirúrgica.</div></div><div class="d-flex gap-2 flex-wrap"><span class="${badgeClassMap[status] || 'badge bg-secondary'}">${statusLabelMap[status] || 'Estado'}</span><span class="badge bg-success">OK: ${Number(summaryData.ok || 0)}</span><span class="badge bg-warning text-dark">Advertencias: ${Number(summaryData.warning || 0)}</span><span class="badge bg-danger">Alertas: ${Number(summaryData.error || 0)}</span></div></div>`;
                checksContainer.innerHTML = checks.map((check) => {
                    ...
                    return `<div class="border rounded p-2 mb-2"><div class="d-flex justify-content-between align-items-start gap-2 flex-wrap"><div><div class="fw-600">${escapeHtml(check.title||'Validación')}</div><div class="small text-muted">${escapeHtml(check.message||'')}</div></div><span class="${badgeClassMap[check.status]||'badge bg-secondary'}">${statusLabelMap[check.status]||'Estado'}</span></div><div class="small mt-2">${details.proyectado?`<div><strong>Proyectado:</strong> ${escapeHtml(details.proyectado)}</div>`:''} ${details.registrado?`<div><strong>Registrado:</strong> ${escapeHtml(details.registrado)}</div>`:''} ${missingList.length>0?`<div><strong>Faltantes:</strong> ${missingList.map(i=>escapeHtml(i)).join(', ')}</div>`:''}</div></div>`;
                }).join('');
            }

            function redirectToEditProtocol() {
                if (!currentFormId || !currentHcNumber) return;
                const endpoints = window.cirugiasEndpoints || {};
                window.location.href = `${endpoints.wizard||'/cirugias/wizard'}?form_id=${encodeURIComponent(currentFormId)}&hc_number=${encodeURIComponent(currentHcNumber)}`;
            }
            ...
            function loadProtocolData(button) {
                ...
                fetch(`${endpoints.protocolo||'/cirugias/protocolo'}?${params.toString()}`)
                    .then(r => r.json())
                    .then(data => {
                        if (data.error) throw new Error(data.error);
                        const diagTable = document.getElementById('diagnostico-table');
                        diagTable.innerHTML = '';
                        (data.diagnosticos||[]).forEach(d => { diagTable.innerHTML += `<tr><td>${d.cie10}</td><td>${d.detalle}</td></tr>`; });
                        const procTable = document.getElementById('procedimientos-table');
                        procTable.innerHTML = '';
                        (data.procedimientos||[]).forEach(p => { procTable.innerHTML += `<tr><td>${p.codigo}</td><td>${p.nombre}</td></tr>`; });
                        const timingRow = document.getElementById('timing-row');
                        timingRow.innerHTML = `<td>${data.fecha_inicio??''}</td><td>${data.hora_inicio??''}</td><td>${data.hora_fin??''}</td><td>${data.duracion??''}</td>`;
                        ...
                        Object.entries(data.staff||{}).forEach(([rol, nombre]) => {
                            if (nombre && nombre.trim()!=='') staffTable.innerHTML += `<tr><td>${rol}</td><td>${nombre}</td></tr>`;
                        });
                        const comment = document.querySelector('.comment-here');
                        if (comment) comment.textContent = data.comentario??'';
                        renderProtocolAudit(data.auditoria);
                    })
                    .catch(() => { Swal.fire('Error', 'No se pudo cargar el protocolo.', 'error'); });
            }
        </script>
