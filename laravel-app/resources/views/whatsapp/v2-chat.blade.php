            ...
            <div class="box mb-0">
                <div class="box-body d-flex flex-wrap justify-content-between align-items-center gap-15">
                    <div>
                        ...
                        <div class="text-muted">Migración gradual sobre Laravel con fallback a legacy y filtros tipo Whatsbox.</div>
                    </div>
                    <div class="text-end">
                        <div class="fw-700">{{ $currentUser['display_name'] ?? 'Usuario' }}</div>
                        <div class="text-muted" style="font-size:12px;">{{ $canSupervise ? 'Modo supervisor' : 'Vista de agente' }}</div>
                    </div>
                </div>
                <div class="box-footer bg-transparent d-flex flex-wrap gap-10 align-items-center justify-content-between">
                    <div class="d-flex align-items-center gap-10">
                        <label for="wa-v2-presence" class="mb-0 text-muted" style="font-size:12px;">Presencia</label>
    ...
    <div class="wa-v2-shell">
            ...
            <div class="wa-v2-panel__header">
                ...
                @if($canSupervise)
                    ...
                    <div class="row g-10 mb-15">
                        <div class="col-sm-6 col-xl-3">
                            <div class="wa-v2-panel p-10" style="border-radius:14px;">
                                ...
                                <div class="fw-700" style="font-size:22px;">{{ (int) ($summaryTotals['queued_open_count'] ?? 0) }}</div>
                            </div>
                        </div>
                        <div class="col-sm-6 col-xl-3">
                            <div class="wa-v2-panel p-10" style="border-radius:14px;">
                                <div class="text-muted" style="font-size:11px;">Chats asignados</div>
                                <div class="fw-700" style="font-size:22px;">{{ (int) ($summaryTotals['assigned_open_count'] ?? 0) }}</div>
                            </div>
                        </div>
                        <div class="col-sm-6 col-xl-3">
                            <div class="wa-v2-panel p-10" style="border-radius:14px;">
                                <div class="text-muted" style="font-size:11px;">Chats con unread</div>
                                <div class="fw-700" style="font-size:22px;">{{ (int) ($summaryTotals['unread_open_count'] ?? 0) }}</div>
                            </div>
                        </div>
                        <div class="col-sm-6 col-xl-3">
                            <div class="wa-v2-panel p-10" style="border-radius:14px;">
                                <div class="text-muted" style="font-size:11px;">TTL por vencer</div>
                                <div class="fw-700" style="font-size:22px;">{{ (int) ($summaryTotals['expiring_soon_count'] ?? 0) }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <form method="GET" action="/v2/whatsapp/chat" class="row g-10 mb-15">
                        ...
                        <div class="col-12">
                            <div class="text-uppercase text-muted mb-5" style="font-size:11px; letter-spacing:.08em;">Filtros supervisor</div>
                        </div>
                        <div class="col-sm-6">
                            <select name="agent_id" class="form-select form-select-sm">
                                <option value="">Todos los agentes</option>
                                <option value="0" {{ $selectedAgentId === 0 ? 'selected' : '' }}>Sin asignar</option>
                                @foreach($agents as $agent)
                                    <option value="{{ $agent['id'] }}" {{ $selectedAgentId === (int) $agent['id'] ? 'selected' : '' }}>
                                        {{ $agent['name'] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12 d-flex gap-8">
                            ...
                            <a href="{{ '/v2/whatsapp/chat?' . http_build_query(['filter' => $selectedFilter, 'search' => $search]) }}" class="btn btn-outline-secondary btn-sm">Limpiar</a>
                        </div>
                    </form>

                    @if(!empty($agentSummary['agents']))
                        <div class="mb-15">
                            <div class="text-uppercase text-muted mb-5" style="font-size:11px; letter-spacing:.08em;">Carga por agente</div>
                                    ...
                                    <div class="wa-v2-panel p-10" style="border-radius:14px;">
                                        <div class="d-flex justify-content-between align-items-center gap-10">
                                            <div>
                                                ...
                                                <div class="text-muted" style="font-size:12px;">{{ $agent['role_name'] ?: 'Sin rol' }} · {{ $agent['presence_status'] }}</div>
                                            </div>
                                            <div class="text-end" style="font-size:12px;">
                                                <div>{{ (int) ($agent['assigned_open_count'] ?? 0) }} asignados</div>
                                                <div>{{ (int) ($agent['unread_open_count'] ?? 0) }} con unread</div>
                                                <div>{{ (int) ($agent['expiring_soon_count'] ?? 0) }} por vencer</div>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                ...
                @forelse($listData as $conversation)
                    ...
                    <a
                        ...
                        <div class="d-flex justify-content-between gap-10">
                            <div class="wa-v2-name">{{ $conversation['display_name'] ?: $conversation['wa_number'] }}</div>
                            <div class="wa-v2-meta">{{ $conversation['unread_count'] > 0 ? $conversation['unread_count'] . ' nuevos' : '' }}</div>
                        </div>
                        ...
                        <div class="wa-v2-preview">{{ $conversation['last_message_preview'] ?: '[' . ($conversation['last_message_type'] ?: 'mensaje') . ']' }}</div>
                        <div class="d-flex flex-wrap gap-8 mt-10">
                            @if((int) $conversation['unread_count'] > 0)
                                <span class="wa-v2-pill is-unread">Sin leer</span>
                            @endif
                            @if(!empty($conversation['needs_human']))
                                <span class="wa-v2-pill is-queue">En cola</span>
                            @else
                                <span class="wa-v2-pill">Resuelto</span>
                            @endif
                            <span class="wa-v2-pill">{{ $conversation['ownership_label'] ?? 'Sin ownership' }}</span>
                            @if(!empty($conversation['assigned_role_name']))
                                <span class="wa-v2-pill">{{ $conversation['assigned_role_name'] }}</span>
                            @endif
                        </div>
                    </a>
                @empty
        ...
        <div class="wa-v2-panel wa-v2-chat">
            @if($selectedConversation)
                ...
                @endphp
                <div class="wa-v2-panel__header">
                    <div class="d-flex flex-wrap justify-content-between gap-15 align-items-start">
                        <div>
                        ...
                        <div class="d-flex flex-wrap gap-8">
                            ...
                            @else
                                <span class="wa-v2-pill">Resuelto</span>
                            @endif
                            <span class="wa-v2-pill">{{ $selectedConversation['ownership_label'] ?? 'Sin ownership' }}</span>
                            @if(!empty($selectedConversation['assigned_role_name']))
                                <span class="wa-v2-pill">{{ $selectedConversation['assigned_role_name'] }}</span>
                            @endif
                        </div>
                    </div>

                    <div class="wa-v2-actions">
                        <a href="{{ $waLink }}" target="_blank" rel="noopener" class="btn btn-light">
                            Abrir en WhatsApp
                        </a>
                        ...
                        @if($canOperateConversation)
                            ...
                            <input type="text" class="form-control" id="wa-v2-transfer-note" placeholder="Nota de transferencia (opcional)">
                            ...
                            <input type="text" class="form-control" id="wa-v2-queue-note" placeholder="Nota para la cola (opcional)">
                ...
                <div class="wa-v2-chat__body">
                    @foreach(($selectedConversation['messages'] ?? []) as $message)
                        <div class="wa-v2-message {{ ($message['direction'] ?? '') === 'outbound' ? 'is-outbound' : '' }}">
                ...
                <div class="wa-v2-compose">
                    <form id="wa-v2-send-form" data-conversation-id="{{ $selectedConversation['id'] }}">
                        <div class="mb-10 text-muted" style="font-size:12px;">
                            Esta fase mantiene reglas legacy: si no hubo mensaje entrante previo se exige plantilla, y si el chat no está tomado no se puede responder.
                        </div>
                        <div class="input-group">
                            <textarea class="form-control" id="wa-v2-message-input" rows="2" placeholder="Escribe una respuesta manual" {{ $canReplyHere ? '' : 'disabled' }}></textarea>
                            <button type="submit" class="btn btn-primary" {{ $canReplyHere ? '' : 'disabled' }}>Enviar</button>
                        </div>
            ...
            @else
                <div class="wa-v2-empty">
                    <div>
                        ...
                        <div>La vista v2 ya soporta filtros operativos, detalle y envío manual sobre los endpoints Laravel.</div>
                    </div>
                </div>
            @endif
        </div>
