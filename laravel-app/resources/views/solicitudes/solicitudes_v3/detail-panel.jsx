/* ============================================================
   MedForge · Solicitudes v2 — Panel CRM (workspace por pestañas)
   ============================================================ */
...
const PRIO_TONE = { "Alta": "danger", "Media": "warning", "Normal": "ok", "Baja": "ok" };
...
function buildTimeline(sol) {
  ...
  return ev.sort((a, b) => new Date(b.time) - new Date(a.time)).slice(0, 8);
}

/* ---------------- Tab: Seguimiento ---------------- */
function TabSeguimiento({ sol, onToggleStep }) {
  ...
  return (
    <React.Fragment>
      ...
      <section>
        <h3 className="psec-title"><i className="mdi mdi-format-list-checks"></i>Checklist operativo
        ...
        <div className="timeline">
          {timeline.map((e, i) => (
            <div className="tl-item" key={i}>
              <span className="tl-dot" style={{ background: e.note ? "var(--info)" : "var(--success)" }}><i className={`mdi ${e.note ? "mdi-note-text-outline" : "mdi-check"}`}></i></span>
              <div className="tl-body"><div className="tl-act">{e.act}</div><div className="tl-time">{e.time}</div><div className="tl-by">{e.by}</div></div>
            </div>
          ))}
        </div>
      </section>
...
/* ---------------- Tab: Tareas ---------------- */
function TabTareas({ sol, onToggleTask, onAddTask }) {
  const [titulo, setTitulo] = React.useState("");
  const [prio, setPrio] = React.useState("Normal");
  const tareas = sol.detalle.tareas;
  const submit = (e) => { e.preventDefault(); if (!titulo.trim()) return; onAddTask(sol.id, { titulo: titulo.trim(), prioridad: prio, asignado: sol.crm.responsable, fecha: new Date(Date.now() + 2 * 86400000).toISOString(), done: false }); setTitulo(""); setPrio("Normal"); };
  return (
    <section>
      <h3 className="psec-title"><i className="mdi mdi-format-list-checks"></i>Tareas y recordatorios
      ...
      <form className="add-form" onSubmit={submit}>
        <input className="fld" placeholder="Nueva tarea (ej. Llamar al paciente)" value={titulo} onChange={(e) => setTitulo(e.target.value)} />
...
/* ---------------- Tab: Notas ---------------- */
function TabNotas({ sol, onAddNote }) {
  const [txt, setTxt] = React.useState("");
  const notas = sol.detalle.notas;
  const submit = (e) => { e.preventDefault(); if (!txt.trim()) return; onAddNote(sol.id, txt.trim()); setTxt(""); };
  return (
    <section>
...
/* ---------------- Tab: Comunicación ---------------- */
function TabComunicacion({ sol, showToast }) {
  const [wa, setWa] = React.useState("");
  const [emailBody, setEmailBody] = React.useState("");
  const sendWa = (e) => { e.preventDefault(); if (!wa.trim()) return; showToast("WhatsApp enviado a " + sol.full_name.split(" ")[0], "mdi-whatsapp"); setWa(""); };
  const sendEmail = (e) => { e.preventDefault(); if (!emailBody.trim()) return; showToast("Correo enviado a " + sol.crm.email, "mdi-email-check-outline"); setEmailBody(""); };
  return (
    <React.Fragment>
      <section>
        <h3 className="psec-title"><i className="mdi mdi-whatsapp" style={{ color: "#1f9d7a" }}></i>WhatsApp
          <span className="psec-meta">{sol.crm.telefono}</span>
        </h3>
...
/* ---------------- Tab: Propuestas ---------------- */
function TabPropuestas({ sol, onAddProposal, showToast }) {
  const props_ = sol.detalle.propuestas;
  const PROP_STATE_TONE = { "Borrador": "none", "Enviada": "warn", "Aceptada": "ok" };
  return (
    <section>
        ...
        {props_.map((p, i) => (
          <div className="prop-card" key={i}>
            <div className="prop-head">
              ...
              <span className={`conc-status conc-${PROP_STATE_TONE[p.estado]}`}>{p.estado}</span>
            </div>
...
/* ---------------- Tab: Documentos ---------------- */
function TabDocumentos({ sol, showToast }) {
      ...
      <section>
        ...
        {der.tiene ? (
          <div className="cover-card">
            ...
            <div className="cover-row"><span>Vigencia</span>
              <span className={`conc-status ${der.vencida ? "conc-warn" : "conc-ok"}`}>
                ...
                {der.vencida ? `Vencida hace ${Math.abs(der.dias_vigencia)} d` : `${der.dias_vigencia} días`}
              </span>
            </div>
...
/* ---------------- Panel ---------------- */
function DetailPanel({ sol, open, onClose, onToggleStep, onAdvance, onToggleTask, onAddTask, onAddNote, onAddProposal, onOpenPrefactura, showToast }) {
  const [tab, setTab] = React.useState("seguimiento");
  React.useEffect(() => { if (open) setTab("seguimiento"); }, [sol && sol.id, open]);
  React.useEffect(() => {
  ...
  const lastDone = sol && sol.estado === "completado";
        ...
        {sol && (
          <React.Fragment>
            ...
            <footer className="panel-foot">
              <button className="btn" title="Abrir prefactura / expediente" onClick={() => onOpenPrefactura(sol.id)}>
              ...
              {!lastDone ? (
                ...
                <button className="btn btn-primary" disabled style={{ opacity: .6 }}><i className="mdi mdi-check-all"></i>Completado</button>
              )}
            </footer>
          </React.Fragment>
        )}
...
Object.assign(window, { DetailPanel, buildTimeline, money });
