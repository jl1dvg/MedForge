/* ============================================================
   MedForge · Solicitudes v2 — Modal Prefactura / Expediente
  ...
  return (
    <React.Fragment>
      <div className="pf-kpis">
        <div className="pf-kpi"><div className={`pf-kpi-ic sla-${sol.sla_status}`}><i className={`mdi ${meta.icon}`}></i></div><div><div className="pf-kpi-v">{fmtSla(sol)}</div><div className="pf-kpi-l">Estado SLA</div></div></div>
        <div className="pf-kpi"><div className="pf-kpi-ic ok"><i className="mdi mdi-format-list-checks"></i></div><div><div className="pf-kpi-v">{sol.checklist_progress.percent}%</div><div className="pf-kpi-l">Checklist operativo</div></div></div>
        <div className="pf-kpi"><div className="pf-kpi-ic info"><i className="mdi mdi-medical-bag"></i></div><div><div className="pf-kpi-v">{preopDone}/{det.preop.length}</div><div className="pf-kpi-l">Preoperatorio</div></div></div>
        <div className="pf-kpi"><div className="pf-kpi-ic money"><i className="mdi mdi-cash-multiple"></i></div><div><div className="pf-kpi-v">{prop ? money(prop.total) : "—"}</div><div className="pf-kpi-l">Propuesta</div></div></div>
      </div>

      <div className="pf-section">
        <h4 className="pf-h">Próximo paso</h4>
        <div className="pf-next"><i className="mdi mdi-arrow-right-circle-outline"></i><b>{sol.checklist_progress.next_label}</b><span>· responsable {sol.crm.responsable}</span></div>
      </div>
...
function PfCobertura({ sol, showToast }) {
  const der = sol.detalle.derivacion;
  if (!der.tiene) {
    return <div className="pf-section"><div className="mini-empty">Paciente particular — no requiere derivación ni autorización de seguro.</div>
      <div className="pf-quick" style={{ marginTop: 14 }}><button onClick={() => showToast("Generando proforma particular", "mdi-cash")}><i className="mdi mdi-cash"></i>Generar proforma particular</button></div>
    </div>;
  }
  return (
    <React.Fragment>
      <div className="pf-section">
        <div className="pf-cover-banner">
          ...
          <span className={`conc-status ${der.vencida ? "conc-warn" : "conc-ok"}`}>
            ...
            {der.vencida ? `Vencida hace ${Math.abs(der.dias_vigencia)} días` : `Vigente · ${der.dias_vigencia} días`}
          </span>
        </div>
      </div>
      {der.autorizacion_pendiente && (
        <div className="pf-section">
          <div className="pf-auth-pending"><i className="mdi mdi-shield-clock-outline"></i>
      ...
      <div className="pf-section">
        ...
        <div className="pf-quick">
          ...
          {der.autorizacion_pendiente && <button className="pf-q-primary" onClick={() => showToast("Solicitud de autorización enviada", "mdi-shield-check")}><i className="mdi mdi-shield-check"></i>Solicitar autorización</button>}
        </div>
      </div>
    </React.Fragment>
  );
...
function PfCirugia({ sol, onTogglePreop }) {
  const preop = sol.detalle.preop;
  const done = preop.filter((p) => p.done).length;
  const pct = Math.round((done / preop.length) * 100);
  return (
    <div className="pf-section">
...
function PfAgenda({ sol, showToast }) {
  const ag = sol.detalle.agenda;
  const programada = ["programada", "completado"].includes(sol.estado);
  return (
    <React.Fragment>
      <div className="pf-section">
        <div className="pf-sigcenter-head"><span className="pf-sig-badge"><i className="mdi mdi-sync"></i>SIGCENTER</span>
...
function PfNota({ sol, showToast }) {
  const ex = sol.detalle.examen;
  const [plan, setPlan] = React.useState(ex.plan);
  return (
    <React.Fragment>
      <div className="pf-section">
      ...
      <div className="pf-section">
        ...
        <button className="btn-add self-end" style={{ marginTop: 10 }} onClick={() => showToast("Nota clínica guardada", "mdi-content-save-outline")}><i className="mdi mdi-content-save-outline"></i>Guardar nota</button>
      </div>
    </React.Fragment>
  );
}

function PrefacturaModal({ sol, open, onClose, onTogglePreop, showToast }) {
  const [tab, setTab] = React.useState("resumen");
  React.useEffect(() => { if (open) setTab("resumen"); }, [sol && sol.id, open]);
  React.useEffect(() => {
  ...
  if (!sol) return <div className={`pf-backdrop ${open ? "open" : ""}`} onClick={onClose}></div>;
...
Object.assign(window, { PrefacturaModal });
