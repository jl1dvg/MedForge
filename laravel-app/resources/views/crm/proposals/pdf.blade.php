...
<head>
    ...
    <style>
        ...
        .header {
            width: 100%;
            border-bottom: 3px solid #2134af;
            padding-bottom: 14px;
            margin-bottom: 20px;
    ...
    <tr>
        <td>
            <div class="box">
                ...
                <div>
                    <strong>Médico:</strong> {{ strtoupper((string) ($clinical['medico'] ?? 'SIN MÉDICO REGISTRADO')) }}
                </div>
                <div><strong>Diagnóstico:</strong> @forelse($diagnosticos as $diagnostico)
        ...
        <td>
            <div class="box">
                <h3 style="margin-top: 0;">Paciente</h3>
                <div><strong>Nombre:</strong> {{ $proposal['lead_name'] ?? $proposal['customer_name'] ?? 'Paciente' }}
                </div>
                <div><strong>HC:</strong> {{ $proposal['lead_hc_number'] ?? '—' }}</div>
                <div>
                    <strong>Seguro:</strong> {{ strtoupper((string) ($clinical['seguro'] ?? 'PARTICULAR / SIN REGISTRO')) }}
                </div>
...
<table class="totals">
    <tr>
        <td>Subtotal</td>
        <td class="right">{{ $fmt($proposal['subtotal'] ?? 0, $currency) }}</td>
    </tr>
    <tr>
        <td>Descuento</td>
        <td class="right">{{ $fmt($proposal['discount_total'] ?? 0, $currency) }}</td>
    </tr>
    <tr>
        <td>Impuesto {{ number_format((float) ($proposal['tax_rate'] ?? 0), 2) }}%</td>
        <td class="right">{{ $fmt($proposal['tax_total'] ?? 0, $currency) }}</td>
    </tr>
    <tr class="total-row">
        <td>Total</td>
        <td class="right">{{ $fmt($proposal['total'] ?? 0, $currency) }}</td>
    </tr>
</table>
