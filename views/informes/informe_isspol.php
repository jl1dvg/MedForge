...
if (isset($_POST['scrape_derivacion']) || (isset($_POST['form_id_scrape']) && isset($_POST['hc_number_scrape']))) {
    ...
    if (!$form_id || !$hc_number) {
        header("Location: ./informe_isspol.php?scrape_exito=1&form_id=" . urlencode($form_id ?? '') . "&hc_number=" . urlencode($hc_number ?? '') . "&msg=" . urlencode("❌ Faltan parámetros requeridos."));
        exit;
    }
    ...
    shell_exec($command);
    header("Location: ./informe_isspol.php?scrape_exito=1&form_id={$form_id}&hc_number={$hc_number}&msg=" . urlencode("✅ Código derivación obtenido y guardado correctamente."));
    exit;
}
